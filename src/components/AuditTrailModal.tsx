...
const opIcon = (op: AuditEntry['operation']) => {
  switch (op) {
    case 'INSERT': return <Plus size={12} className="text-emerald-600" />;
    case 'UPDATE': return <Edit2 size={12} className="text-blue-600" />;
    case 'DELETE': return <Trash2 size={12} className="text-red-600" />;
    case 'RESTORE': return <RefreshCw size={12} className="text-purple-600" />;
  }
};
  ...
  return (
    <AnimatePresence>
      <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
        ...
        >
          <div className="px-6 py-4 border-b border-neu-gold-pale flex items-center justify-between"
          ...
          <div className="px-4 py-3 border-b border-neu-gold-pale bg-neu-cream/50">
            <p className="text-xs text-neu-slate">MOA: <strong className="text-neu-navy">{moa.companyName}</strong></p>
            <p className="text-xs text-neu-slate">HTE ID: {moa.hteId}</p>
          </div>

          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {sorted.length === 0 && (
              <p className="text-center text-sm text-neu-slate py-8">No audit records found</p>
            )}
            {sorted.map((entry, i) => (
              <motion.div
                key={i}
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    <span className={`gold-badge text-xs border ${opColor(entry.operation)}`}>
                      {opIcon(entry.operation)}
                      {entry.operation}
                    </span>
                    <span className="text-xs font-medium text-neu-navy">{entry.userName}</span>
                  </div>
                ...
                {entry.changes && Object.keys(entry.changes).length > 0 && (
                  <div className="mt-2 space-y-1">
                    {Object.entries(entry.changes).map(([field, change]) => (
                      <div key={field} className="text-xs p-1.5 rounded-lg bg-neu-cream">
                        <span className="font-medium text-neu-navy capitalize">{field}</span>
                        <span className="text-red-500 ml-2">"{String(change.before)}"</span>
