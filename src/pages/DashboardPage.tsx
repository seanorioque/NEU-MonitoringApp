...
export const DashboardPage: React.FC<Props> = ({ moas, userRole }) => {
  ...
  const filtered = useMemo(() => {
    return moas.filter((m) => {
      ...
      if (userRole === "student" && getStatusGroup(m.status) !== "APPROVED")
        return false;
  ...
  const stats = useMemo(
    () => ({
      approved: filtered.filter((m) => getStatusGroup(m.status) === "APPROVED")
        .length,
      processing: filtered.filter(
        (m) => getStatusGroup(m.status) === "PROCESSING",
      ).length,
      expiring: filtered.filter((m) => getStatusGroup(m.status) === "EXPIRING")
        .length,
      expired: filtered.filter((m) => getStatusGroup(m.status) === "EXPIRED")
        .length,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {stats.total} MOA{stats.total !== 1 ? "s" : ""} ·{" "}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <span style={{ fontSize: "12px", color: "#8B7355" }}>
            {stats.total} total records shown
          </span>
        </div>
      </div>
      ...
      <div className="section-card" style={{ padding: "14px 16px" }}>
        ...
        >
          ...
          <select
            value={collegeFilter}
            onChange={(e) =>
              setCollegeFilter(e.target.value as College | "ALL")
            }
            className="neu-input"
      ...
      >
        {statCards.map(
          ({ label, value, icon: Icon, accent, lightBg, border }, i) => (
            <motion.div
              key={label}
            ...
            >
              ...
              >
                {stats.total ? Math.round((value / stats.total) * 100) : 0}% of
                total
              </p>
            </motion.div>
          ),
            ...
            >
              {byCollege.map(([college, count]) => (
                <div key={college}>
                  ...
                  >
                    <span style={{ fontWeight: 600, color: "#2D2416" }}>
                      {college}
                    </span>
                  ...
                  >
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(count / (byCollege[0]?.[1] || 1)) * 100}%`,
                      }}
          ...
          {userRole !== "student" && stats.expiring > 0 && (
            ...
            >
              ...
              <p style={{ fontSize: "12px", color: "#C2410C" }}>
                <strong>{stats.expiring}</strong> MOA
                {stats.expiring > 1 ? "s are" : " is"} expiring within 2 months
              </p>
            </div>
          )}
        </motion.div>
