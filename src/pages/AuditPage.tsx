...
export const AuditPage: React.FC<Props> = ({ moas }) => {
  ...
  const [opFilter, setOpFilter] = useState<AuditEntry["operation"] | "ALL">(
    "ALL",
  );
  ...
  const filtered = useMemo(() => {
    ...
    return allAudits.filter((a) => {
      ...
      if (dateTo && new Date(a.timestamp) > new Date(dateTo + "T23:59:59"))
        return false;
      if (!q) return true;
      return [a.userName, a.userEmail, a.moaCompany, a.moaHteId].some((v) =>
  ...
  const stats = {
    ...
    INSERT: allAudits.filter((a) => a.operation === "INSERT").length,
    UPDATE: allAudits.filter((a) => a.operation === "UPDATE").length,
    DELETE: allAudits.filter((a) => a.operation === "DELETE").length,
    RESTORE: allAudits.filter((a) => a.operation === "RESTORE").length,
  };
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div>
            ...
            <p className="page-subtitle">
              All system activity across all MOA records
            </p>
          </div>
        </div>
      </div>
      ...
      >
        {(
          [
            ["Total Events", stats.total, "#0B1B3D", "#F0EAE0"],
            ["Inserts", stats.INSERT, "#16A34A", "#F0FDF4"],
            ["Updates", stats.UPDATE, "#2563EB", "#EFF6FF"],
            ["Deletions", stats.DELETE, "#DC2626", "#FEF2F2"],
            ["Restores", stats.RESTORE, "#9333EA", "#FAF5FF"],
          ] as const
      ...
      >
        ...
        <div style={{ position: "relative", flex: 1, minWidth: "200px" }}>
          ...
          <input
            ...
            placeholder="Search user, email, company, HTE ID…"
            className="neu-input"
      ...
      >
                          ...
                          {Object.entries(entry.changes).map(
                            ([field, change]) => (
                              ...
                              >
                                ...
                                >
                                  {String(change.before).slice(0, 25)}
                                </span>
                                <span
                                  style={{ color: "#8B7355", margin: "0 4px" }}
                                >
                                  →
                                </span>
                                <span style={{ color: "#16A34A" }}>
                                  {String(change.after).slice(0, 25)}
                                </span>
                              </div>
                            ),
                          )}
        ...
        >
          <span style={{ fontSize: "12px", color: "#8B7355" }}>
            ...
            <strong>{allAudits.length}</strong> audit entries
          </span>
        </div>
      </div>
    </div>
